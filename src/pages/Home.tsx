    ...
    return (
        <div className="mx-auto h-[cal(100vh-144)] flex flex-col items-center justify-center bg-gray-900">
            ...
            <div className='pt-56 md:pt-96 lg:pt-72 flex flex-col justify-center'>
                <motion.h1 initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.75 } }} className='font-mono text-left text-xs pl-16 text-white font-bold md:text-xl md:pl-36 lg:pl-28'>
                    Yo soy
                </motion.h1>
            ...
            <div className='pt-36 md:pt-96 lg:pt-80 flex flex-col items-center justify-center'>
                <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.75, delay: 1.25 } }} className='font-mono pt-5 w-80 md:w-[45rem] lg:w-[80rem] text-xs text-white text-center md:text-xl '>
                    "El deseo de conocimiento forma a un hombre ― Patrick Rothfuss, The Wise Man's Fear"
                </motion.h1>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0, transition: { duration: 0.75, delay: 1.50 } }} className='h-16 w-16 flex items-center justify-center'>
            ...
            <div className='mt-20 mb-20 flex flex-col items-center text-white'>
                <motion.h1 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0, transition: { duration: 0.75, delay: 0.75 } }} className='uppercase font-bold pb-5 md:pb-10 md:text-2xl'>competencias</motion.h1>
                <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0, transition: { duration: 0.75, delay: 1 } }} className='w-80 py-5 md:w-[40rem] lg:w-[60rem] grid grid-rows-4 md:grid-rows-3 lg:grid-rows-2 grid-flow-col gap-2 bg-gray-800 shadow-xl justify-center rounded-lg'>
                    {skills.map((item, i) => {
                        return (
                            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1, transition: { duration: 0.75, delay: 1.25 } }} className='flex justify-center' key={i + id}>
